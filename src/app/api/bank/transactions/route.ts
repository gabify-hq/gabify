    ...
    data: {
      items: page.map((t) => ({
        ...
        suggestions: t.suggestions.map((s) => ({
          ...
          supplierName: s.document.supplierName,
          issueDate: s.document.issueDate ? s.document.issueDate.toISOString().slice(0, 10) : null,
          // Money stays integer cents / decimal string — never a JS float (A1)
          totalAmount: s.document.totalAmount !== null ? String(s.document.totalAmount) : null,
        })),
      })),
