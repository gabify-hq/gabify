...
| UI `/assistant`: chat mobile-first pt-PT, histórico só em memória (zero schema), 4 perguntas sugeridas, tabelas server-data com badges de estado, links "ver na fila/documentos/banco" por query params existentes, export CSV client-side (BOM/`;`/vírgula — A9), estados loading/empty/error com retry; link "Assistente" na sidebar (append-only) + 4 testes de componente jsdom | DONE | — | ✅ 4/4 componente + tsc 0 + eslint 0 |
| Docs: `docs/technical/assistant.md` (módulo novo), secção "Ask Gabify" no fim de `docs/HOW_IT_WORKS.md` (append-only), `HANDOFF_ASSISTANT.md` | DONE | — | ✅ |

**Fecho do módulo: `npm run gate` verde — 377 testes (39 ficheiros: 349 base + 24 acceptance assistant + 4 componente), tsc 0 erros, eslint 0 erros, coverage thresholds mantidos (queues ≥80, api ≥70, server ≥75). SEM merge, SEM push.**
