...
/**
 * AUDIT F3.11 â€” CSV de export correto (A-1/A-2).
 *  - RFC 4180: campos com ; " ou quebras de linha vÃªm entre aspas (aspas
 *    duplicadas) â€” um fornecedor "A;B" desalinhava TODAS as colunas.
 *  - Taxas das regiÃµes autÃ³nomas (AÃ§ores 4/9/16, Madeira 5/12/22) NUNCA sÃ£o
 *    silenciosamente omitidas â€” ganham colunas prÃ³prias no lancamentos.csv.
 */

/** Splitter RFC 4180 mÃ­nimo para as asserÃ§Ãµes (aspas duplas escapadas). */
...
describe('AUDIT-F3.11 export CSV', () => {
  ...
  it('campos com ; e aspas ficam corretamente escapados â€” colunas nunca desalinham', async () => {
    const { officeA, ownerA } = await makeTwoOffices()
    const client = await makeClient({ officeId: officeA.id, name: 'Cliente; Com Ponto-e-VÃ­rgula' })
    await makeExportDoc({
      officeId: officeA.id,
      clientId: client.id,
      number: 'FT ESC/1',
    ...
    const csv = zip.getEntry('lancamentos.csv')!.getData().toString('utf-8').replace(/^﻿/, '')
    const lines = csv.split('\n').filter((l) => l !== '' && !l.startsWith('#'))
    const header = splitCsvLine(lines[0])
    const dataLine = splitCsvLine(lines[1])

    // Mesmo nÂº de colunas no header e na linha â€” o ; do fornecedor nÃ£o parte nada
  ...
  it('fatura dos AÃ§ores a 16% aparece no CSV com base e IVA exatos â€” nunca a zeros', async () => {
    const { officeA, ownerA } = await makeTwoOffices()
    const client = await makeClient({ officeId: officeA.id, name: 'Cliente AÃ§ores' })
    await makeExportDoc({
      officeId: officeA.id,
      clientId: client.id,
      number: 'FT AC/1',
      supplierName: 'Fornecedor AÃ§oriano Lda',
    ...
    // Colunas dinÃ¢micas para a banda fora do continente
    ...
    // O dinheiro nÃ£o desapareceu para as colunas continentais
    expect(dataLine).toContain('464,00') // total
    // resumo_iva inclui a taxa com a regiÃ£o
